...
async function run() {
  try {
    ...
    const existing = await Product.findOne({ sku: "FB-STORED-IMG-TEST-001" });
    ...
    await VendorMapping.deleteMany({
      ...
      vendorPartNumber: { $in: ["BB-STORED-IMG-001"] },
    });
    ...
    const upsertResult = await upsertProductFromFishbowl({
      partId: "FB-STORED-IMG-1001",
      partNum: "FB-STORED-IMG-PART-001",
      sku: "FB-STORED-IMG-TEST-001",
      internalPartNumber: "IHI-STORED-IMG-001",
      uom: "ea",
      status: "Active",
      type: "Inventory",
    ...
    const brightonRows = [
      {
        partNumber: "BB-STORED-IMG-001",
        description: '5/16-18 x 2" Grade 8 Hex Bolt Yellow Zinc',
        category: "Hex Bolts",
        internalPartNumber: "IHI-STORED-IMG-001",
        websiteSku: "FB-STORED-IMG-TEST-001",
        available: 45,
        onHand: 50,
        onOrder: 20,
        allocated: 5,
    ...
    const imagePath =
      finalEnrichment?.images?.[0]?.url
        ? path.resolve("." + finalEnrichment.images[0].url)
        : null;

    const fileExists = imagePath ? fs.existsSync(imagePath) : false;
    ...
    console.log(JSON.stringify({ imagePath, fileExists }, null, 2));

    process.exit(0);
  } catch (error) {
    console.error("Vendor image storage test failed:", error);
    process.exit(1);
  }
}
