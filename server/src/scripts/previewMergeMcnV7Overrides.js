...
const PREVIEW_PATH = path.resolve(
	process.cwd(),
	"tmp",
	"manufacturer-matching-mcn-overrides-v7-preview.csv",
);

const AUDIT_PATH = path.resolve(
	process.cwd(),
	"tmp",
	"manufacturer-matching-mcn-overrides-v7-merge-audit.csv",
);

const SUMMARY_PATH = path.resolve(
	process.cwd(),
	"tmp",
	"manufacturer-matching-mcn-overrides-v7-merge-summary.json",
);
...
function main() {
	...
	if (
		passed &&
		summary.existingOverrides === 982 &&
		summary.newAdditions === 53 &&
		summary.added === 53 &&
		summary.finalPreviewRows === 1035
	) {
		console.log("✅ V7 merge preview passed perfectly.");

		console.log("✅ 982 existing + 53 new = 1,035 verified MCN overrides.");
