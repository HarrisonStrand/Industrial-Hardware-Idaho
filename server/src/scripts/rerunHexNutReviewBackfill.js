...
async function recomputeAndPersist(productId) {
	...
	if (!product || !enrichment) return { status: "skipped", productId };
	...
	return { status: "ok", reviewStatus: nextStatus };
}

async function main() {
	console.time("hexNutReviewBackfill");
	...
	const enrichments = await ProductEnrichment.find(
		{
			$or: [
				{ category: /^nuts$/i, subcategory: /^hex nuts$/i },
				{ "attributes.familyType": /^hex nut$/i },
				{ "attributes.familyType": /^heavy hex nut$/i },
				{ "attributes.fastenerTypeCanonical": /^hex nut$/i },
				{ "attributes.fastenerTypeCanonical": /^heavy hex nut$/i },
			],
		},
		{ productId: 1 },
	).lean();
	...
	console.log(`Found ${productIds.length} hex nut enrichment${productIds.length === 1 ? "" : "s"}`);
	...
	for (const productId of productIds) {
		try {
			...
			if (processed % 100 === 0) console.log(`Processed ${processed}/${productIds.length}`);
	...
	console.log("===== HEX NUT REVIEW SUMMARY =====");
	console.log(summary);

	await mongoose.disconnect();
	console.log("✅ Done");
	console.timeEnd("hexNutReviewBackfill");
}

main().catch(async (err) => {
	console.error("❌ Hex nut review backfill failed:", err);
	try {
		await mongoose.disconnect();
	} catch {}
	process.exit(1);
