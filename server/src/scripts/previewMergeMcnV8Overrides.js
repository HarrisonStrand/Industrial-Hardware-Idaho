...
const PREVIEW_PATH = path.resolve(
	process.cwd(),
	"tmp",
	"manufacturer-matching-mcn-overrides-v8-preview.csv",
);

const AUDIT_PATH = path.resolve(
	process.cwd(),
	"tmp",
	"manufacturer-matching-mcn-overrides-v8-merge-audit.csv",
);

const SUMMARY_PATH = path.resolve(
	process.cwd(),
	"tmp",
	"manufacturer-matching-mcn-overrides-v8-merge-summary.json",
);
...
function main() {
	...
	if (
		passed &&
		summary.existingOverrides === 1035 &&
		summary.newAdditions === 121 &&
		summary.added === 121 &&
		summary.finalPreviewRows === 1156
	) {
		console.log("✅ V8 merge preview passed perfectly.");

		console.log("✅ 1,035 existing + 121 new = 1,156 verified MCN overrides.");
