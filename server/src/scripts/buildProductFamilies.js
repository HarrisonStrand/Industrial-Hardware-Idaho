...
function buildFamilyKey(product) {
  ...
  const category = firstNonEmpty(parsed.category, product?.categoryHints?.[0]);
  const subcategory = firstNonEmpty(parsed.subcategory, product?.categoryHints?.[1]);
...
function buildFamilyShortTitle(group = []) {
  ...
  const parts = [parsed.finish, parsed.fastenerType]
    .map(clean)
    .filter(Boolean);
...
function buildFamilyDescription(group = []) {
  ...
  const category = firstNonEmpty(parsed.category, sample?.categoryHints?.[0]);
  const subcategory = firstNonEmpty(parsed.subcategory, sample?.categoryHints?.[1]);
...
function buildFamilyMeta(group = []) {
  ...
  return {
    ...
    category: firstNonEmpty(parsed.category, sample?.categoryHints?.[0]),
    subcategory: firstNonEmpty(parsed.subcategory, sample?.categoryHints?.[1]),
...
async function applyFamilyToGroup(group = []) {
  ...
  for (const product of group) {
    ...
    enrichment.shortDescription =
      enrichment.shortDescription ||
      `${family.familyTitle}${
        product?.fishbowl?.partNum ? ` (${product.fishbowl.partNum})` : ""
      }`;
...
export async function buildProductFamilies({
  ...
  if (category || subcategory) {
    query.categoryHints = { $exists: true, $ne: [] };
  }
  ...
  let filtered = products.filter((product) => {
    const parsed = getParsed(product);
    const pCategory = firstNonEmpty(parsed.category, product?.categoryHints?.[0]);
    const pSubcategory = firstNonEmpty(parsed.subcategory, product?.categoryHints?.[1]);

    if (category && clean(category).toLowerCase() !== pCategory.toLowerCase()) {
      return false;
    }

    if (subcategory && clean(subcategory).toLowerCase() !== pSubcategory.toLowerCase()) {
      return false;
    }
