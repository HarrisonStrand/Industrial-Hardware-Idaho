...
const PREVIEW_PATH = path.resolve(
  process.cwd(),
  "tmp",
  "manufacturer-matching-mcn-overrides-v6-preview.csv"
);

const AUDIT_PATH = path.resolve(
  process.cwd(),
  "tmp",
  "manufacturer-matching-mcn-overrides-v6-merge-audit.csv"
);

const SUMMARY_PATH = path.resolve(
  process.cwd(),
  "tmp",
  "manufacturer-matching-mcn-overrides-v6-merge-summary.json"
);
...
function main() {
  ...
  for (
    ...
    if (
      ...
      auditRows.push(
        makeAuditRow({
          ...
          notes:
            `Internal ID already belongs to part number "${clean(
              existing[
                "IHI Part Number"
              ]
            )}".`,
    ...
    auditRows.push(
      makeAuditRow({
        ...
        notes:
          "New verified Hindley v6 MCN override added to preview.",
      })
    );
  }
  ...
  console.log(
    `New v6 additions:            ${summary.newAdditions}`
  );
  ...
  if (
    passed &&
    summary.existingOverrides === 821 &&
    summary.newAdditions === 161 &&
    summary.added === 161 &&
    summary.finalPreviewRows === 982
  ) {
    console.log(
      "✅ V6 merge preview passed perfectly."
    );

    console.log(
      "✅ 821 existing + 161 new = 982 verified MCN overrides."
    );
    ...
    console.log(
      "ℹ️ Counts differ from the expected 821 + 161 checkpoint, so review before promotion."
    );
  } else {
    console.log(
