...
function detectCompactMetricCapScrew(text = "", product = null) {
	...
	if (!partNum.startsWith("MM") && !/mm/i.test(raw)) return null;

	const patterns = [
		/C\/S\s+(\d+(?:\.\d+)?)MMX(\d+(?:\.\d+)?)-(\d+(?:\.\d+)?)(?:\s*(S\/S|SS))?/i,
		/C\/S\s+(\d+(?:\.\d+)?)X(\d+(?:\.\d+)?)MM[\s-]?(\d+(?:\.\d+)?)(?:\s*(S\/S|SS))?/i,
		/(\d+(?:\.\d+)?)X(\d+(?:\.\d+)?)MM[\s-]?(\d+(?:\.\d+)?)(?:\s*(S\/S|SS))?/i,
		/C\/S\s+(\d+(?:\.\d+)?)X(\d+(?:\.\d+)?)[\s]+(\d+(?:\.\d+)?)(?:\s*(PL|ZC|S\/S|SS))?/i,
	];
	for (const pattern of patterns) {
		const m = raw.match(pattern);
		if (!m) continue;
		const [, d='', l='', pitch='', suffix=''] = m;
		const isStainless = /S\/S|SS/i.test(suffix) || /SS$/i.test(partNum) || /S\/S/i.test(raw);
		return {
			measurementSystem: 'metric',
			diameter: `M${clean(d)}`,
			length: `${clean(l)}mm`,
			...
			finish: isStainless ? '' : 'zinc',
			grade: isStainless ? (/316/i.test(raw) ? '316' : '304') : (/12\.9/i.test(raw) ? '12.9' : (/10\.9/i.test(raw) ? '10.9' : '8.8')),
		};
	}
	return null;
}
...
function detectRoundStockA307(text = '') {
	const raw = String(text || '');
	if (!/round stock/i.test(raw) || !/A307/i.test(raw)) return null;
...
function detectBoltLikeFamily(text = "", parsed = {}, product = null) {
	...
	let threadPitch =
		...
		(/DOM|DOMESTIC/i.test(text) && decodedPitch ? "" : imperialTextPitch) ||
		metricPitch ||
