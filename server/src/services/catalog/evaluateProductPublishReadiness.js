...
function canonicalFamilyKeyFromAttributes(enrichment) {
	...
	const category = normalize(
		enrichment?.category || attrs.categoryCanonical || "",
	);
...
async function findSimilarFamilyCandidates(enrichment) {
	...
	for (const candidate of candidates) {
		...
		const { score, reasons } = compareAttributesScore(
			attrs,
		...
		if (!existing || existing.score < score) {
			byFamilyKey.set(familyKey, {
				familyKey,
				familyTitle:
					candidate?.attributes?.familyTitle || candidate?.title || "",
				score,
...
export async function evaluateProductPublishReadiness(productId, options = {}) {
	...
	const description = clean(
		enrichment.description || enrichment.shortDescription || "",
	);
	...
	if (isLikelyAssemblyText(textToInspect)) {
		pushIssue(issues, {
			...
			field: "description",
			message:
				"This appears to be an assembly-style item and is not safe for builder rendering.",
		});
	}
