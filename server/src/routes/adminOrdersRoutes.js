...
async function getOrderSummary() {
	const [adminAgg, paymentAgg, fishbowlAgg, total] = await Promise.all([
		Order.aggregate([
			{ $group: { _id: "$adminReview.status", count: { $sum: 1 } } },
		]),
		Order.aggregate([
			{ $group: { _id: "$payment.status", count: { $sum: 1 } } },
		]),
		Order.aggregate([
			{ $group: { _id: "$fishbowlStatus", count: { $sum: 1 } } },
		]),
		Order.countDocuments({}),
	]);

	const admin = Object.fromEntries(
		adminAgg.map((row) => [row._id || "PENDING", row.count]),
	);
	const payment = Object.fromEntries(
		paymentAgg.map((row) => [row._id || "PENDING", row.count]),
	);
	const fishbowl = Object.fromEntries(
		fishbowlAgg.map((row) => [row._id || "NOT_SENT", row.count]),
	);
...
router.get("/orders/summary", requireAuth, requireAdmin, async (_req, res) => {
	try {
		...
		res
			.status(500)
			.json({ error: err?.message || "Failed to summarize orders" });
	}
});
	...
	async (req, res) => {
		try {
			...
			if (status === "DENIED" && !reason) {
				return res
					.status(400)
					.json({ error: "deniedReason is required when status is DENIED" });
			}
			...
			if (
				["APPROVED_IN_PROGRESS", "APPROVED_COMPLETED"].includes(status) &&
				...
				return res
					...
					.json({
						error:
							"Orders with failed payment cannot be approved or completed.",
					});
			}
			...
			res
				.status(500)
				.json({ error: err?.message || "Failed to update order review" });
		}
	},
);
		...
		try {
			...
			if (paymentStatus === "FAILED") {
				return res
					.status(400)
					.json({
						error: "Cannot push an order with failed payment to Fishbowl.",
					});
			}
			...
			if (reviewStatus === "DENIED") {
				return res
					.status(400)
					.json({ error: "Cannot push a denied order to Fishbowl." });
			}
			...
			if (reviewStatus === "PENDING") {
				return res
					...
					.json({
						error:
							"Approve/start processing this order before pushing it to Fishbowl.",
					});
			}
			...
			if (result?.pushed === false) {
				return res
					...
					.json({
						ok: false,
						result,
						error: result.error || "Fishbowl push failed",
					});
			}
			...
			res
				.status(500)
				.json({ error: err?.message || "Failed to push order to Fishbowl" });
		}
	},
);
