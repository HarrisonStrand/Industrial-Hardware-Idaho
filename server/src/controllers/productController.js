...
async function recomputeAndPersist(productId, userId = null) {
  ...
  enrichment.quality = {
    ...
    similarFamilies: readiness.similarFamilyCandidates.map((candidate) => ({
      ...
      reasons: candidate.reasons || (candidate.reason ? [candidate.reason] : []),
    })),
    suggestedFamilyKey: readiness.suggestedFamilyKey || "",
...
async function buildFilteredAdminRows({
  ...
  if (subcategory) enrichmentFilter.subcategory = normalizeQueryValue(subcategory);
  if (familyType) enrichmentFilter["attributes.familyType"] = normalizeQueryValue(familyType);
...
export const getAdminReviewSummary = async (req, res) => {
  try {
    ...
    for (const item of enrichments) {
      ...
      if (subcategory) subcategoryMap.set(subcategory, (subcategoryMap.get(subcategory) || 0) + 1);
      if (familyType) familyTypeMap.set(familyType, (familyTypeMap.get(familyType) || 0) + 1);
    }
    ...
    if (search) {
      ...
      enrichmentMatchExpr.push({
        $or: [
          ...
          { $regexMatch: { input: { $ifNull: ["$shortTitle", ""] }, regex, options: "i" } },
          { $regexMatch: { input: { $ifNull: ["$category", ""] }, regex, options: "i" } },
          { $regexMatch: { input: { $ifNull: ["$subcategory", ""] }, regex, options: "i" } },
          { $regexMatch: { input: { $ifNull: ["$attributes.familyType", ""] }, regex, options: "i" } },
          { $regexMatch: { input: { $ifNull: ["$attributes.fishbowlPartNum", ""] }, regex, options: "i" } },
          { $regexMatch: { input: { $ifNull: ["$seo.slug", ""] }, regex, options: "i" } },
        ],
      });
    }
...
export const patchAdminProduct = async (req, res) => {
  try {
    ...
    if (!product || !enrichment) {
      return res.status(404).json({ message: "Product or enrichment not found" });
    }

    if (Object.prototype.hasOwnProperty.call(productPayload, "isActive")) {
      product.isActive = Boolean(productPayload.isActive);
    }

    if (productPayload.brand !== undefined) {
      product.brand = String(productPayload.brand || "");
    }

    if (productPayload.vendor !== undefined) {
      product.vendor = String(productPayload.vendor || "");
    }

    if (productPayload.pricing) {
      const pricing = safeObject(productPayload.pricing);
    ...
    const assignString = (key) => {
      if (Object.prototype.hasOwnProperty.call(enrichmentPayload, key)) {
        enrichment[key] = String(enrichmentPayload[key] || "");
      }
    };
    ...
    if (Array.isArray(enrichmentPayload.tags)) {
      enrichment.tags = enrichmentPayload.tags
        .map((item) => String(item || "").trim())
        .filter(Boolean);
    }

    if (enrichmentPayload.attributes && typeof enrichmentPayload.attributes === "object") {
      enrichment.attributes = {
        ...(enrichment.attributes || {}),
        ...safeObject(enrichmentPayload.attributes),
      };
    }

    if (enrichmentPayload.seo && typeof enrichmentPayload.seo === "object") {
      enrichment.seo = {
        ...(enrichment.seo?.toObject?.() || enrichment.seo || {}),
        ...safeObject(enrichmentPayload.seo),
      };
    }
...
export const approveAdminProduct = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(400).json({ message: err.message || "Failed to approve product" });
  }
};
...
export const publishAdminProduct = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(400).json({ message: err.message || "Failed to publish product" });
  }
};
...
export const unpublishAdminProduct = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(400).json({ message: err.message || "Failed to unpublish product" });
  }
};
...
export const recomputeAdminProduct = async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(400).json({ message: err.message || "Failed to recompute product readiness" });
  }
};